...
def get_platform_monthly_revenue():
    ...
    revenue = Purchase.objects.filter(created_at__month=now.month, created_at__year=now.year, status="paid").aggregate(
        commission=Sum("commission"), fees=Sum("checkout_fee")
    )
...
def get_dj_monthly_earnings(dj_id):
    ...
    earnings = Purchase.objects.filter(
        seller_id=dj_id, status="paid", download_completed=True, created_at__month=now.month, created_at__year=now.year
...
def get_dj_lifetime_earnings(dj_id):
    ...
    lifetime = Purchase.objects.filter(seller_id=dj_id, status="paid", download_completed=True).aggregate(
...
def get_best_month_earnings(dj_id):
    ...
    monthly_stats = (
        Purchase.objects.filter(seller_id=dj_id, status="paid", download_completed=True)
        .annotate(month=TruncMonth("created_at"))
        .values("month")
        .annotate(total=Sum("dj_earnings"))
        .order_by("-total")
    )
