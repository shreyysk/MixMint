    ...
    def check_and_award(cls, dj_profile):
        ...
        for milestone, achieved in milestones_to_check:
            if achieved and not cls.objects.filter(dj=dj_profile, milestone=milestone).exists():
                reward = cls.objects.create(dj=dj_profile, milestone=milestone, reward_amount=cls.REWARDS[milestone])
...
class DJPromoCode(models.Model):
    ...
    def is_valid(self):
        ...
        now = timezone.now()
        return self.is_active and self.valid_from <= now <= self.valid_until and self.times_used < self.max_uses
    ...
    def apply_to_dj(self, dj_profile):
        ...
        DJPromoApplication.objects.create(
            dj=dj_profile, promo=self, commission_discount=self.commission_discount, expires_at=self.valid_until
        )
