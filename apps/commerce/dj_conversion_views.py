...
from .dj_conversion import DJReferralProgram, DJMilestoneReward, DJPromoCode, DJOnboardingProgress
...
def get_referral_code(request):
    ...
    code, created = AmbassadorCode.objects.get_or_create(
        dj=dj_profile, defaults={"code": f"DJ{secrets.token_hex(4).upper()}"}
    )
    ...
    instagram_bio_text = (
        f"🎧 Sell your tracks on MixMint\n💰 Use code: {code.code}\n🎁 Get ₹50 bonus!\n👇 {referral_url}"
    )
    twitter_url = f"https://twitter.com/intent/tweet?text={share_message}&url={referral_url}".replace(" ", "%20")
    facebook_url = f"https://www.facebook.com/sharer/sharer.php?u={referral_url}&quote={share_message}".replace(
        " ", "%20"
    )
...
def get_onboarding_status(request):
    ...
    steps = [
        ...
        {
            "id": "track",
            "name": "Upload your first track",
            "completed": progress.first_track_uploaded,
            "tip": "Get ₹50 welcome bonus after uploading!",
        },
        {
            "id": "pricing",
            "name": "Set custom pricing",
            "completed": progress.pricing_set,
...
def get_dj_dashboard_stats(request):
    ...
    this_month_sales = Purchase.objects.filter(
        seller=dj_profile, status="paid", created_at__month=timezone.now().month, created_at__year=timezone.now().year
    ).count()
    ...
    # Referral stats
    referral_earnings = DJReferralProgram.objects.filter(referrer=dj_profile, referrer_paid=True).count() * 100
