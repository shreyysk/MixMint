...
@transaction.atomic
def create_purchase(
    user_profile, content_id, content_type, payment_id, payment_order_id=None, is_redownload=False, **kwargs
):
    """
    ...
    invoice_enabled = (
        invoice_setting_obj.value.get("enabled", True)
        if invoice_setting_obj and isinstance(invoice_setting_obj.value, dict)
        else True
    )
    ...
    try:
        ...
        html_lines.append(
            "<p>You can re-download your purchase from your MixMint library, "
...
def check_high_value_transaction(purchase):
    ...
    recent_purchases = Purchase.objects.filter(
        user_id=user_id, created_at__gte=timezone.now() - timedelta(hours=1), status__in=["active", "pending"]
    )
