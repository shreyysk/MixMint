...
def release_escrow_funds():
    ...
    purchases_to_release_standard = Purchase.objects.filter(
        status="paid", paid_at__lte=standard_cutoff, seller__is_verified=False, is_escrow_released=False
    )
...
def place_earnings_hold(dj_id, hold_type, amount=None, reason="", admin_id=None, notify_dj=True):
    ...
    with transaction.atomic():
        ...
        EarningsHold.objects.create(
            dj_id=dj_id, amount=hold_amount, hold_type=hold_type, reason=reason, placed_by_id=admin_id, status="active"
        )
    ...
    if notify_dj:
        try:
            ...
            send_email(
                ...
                html_content=f"<p>A hold of ₹{hold_amount} has been placed on your earnings for {
                    hold_type.replace(
                        '_',
                        ' ')}. Please contact support for more details.</p>",
            )
        except Exception:
            pass
