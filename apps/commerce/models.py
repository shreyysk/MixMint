...
class TransactionAlert(models.Model):
    ...
    reviewed_by = models.ForeignKey(
        Profile, on_delete=models.SET_NULL, null=True, blank=True, related_name="reviewed_tx_alerts"
    )
    outcome = models.CharField(
        max_length=30,
        null=True,
        blank=True,
        choices=(("safe", "Safe"), ("frozen", "Frozen"), ("refunded_blocked", "Refunded & Blocked")),
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class EarningsHold(models.Model):
    ...
    status = models.CharField(
        max_length=20,
        default="active",
        choices=(("active", "Active"), ("released", "Released"), ("forfeited", "Forfeited")),
    )
    placed_by = models.ForeignKey(
        Profile, on_delete=models.SET_NULL, null=True, blank=True, related_name="holds_placed"
    )
    resolved_by = models.ForeignKey(
        Profile, on_delete=models.SET_NULL, null=True, blank=True, related_name="holds_resolved"
    )
