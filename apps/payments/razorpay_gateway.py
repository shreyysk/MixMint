...
class RazorpayGateway(PaymentGateway):
    ...
    def create_order(self, amount_paise, currency="INR", order_id=None, metadata=None):
        notes = metadata or {}
        data = {"amount": int(amount_paise), "currency": currency, "receipt": order_id, "notes": notes}
        order = self.client.order.create(data=data)
        return {"success": True, "order_id": order["id"], "amount": order["amount"], "gateway_response": order}
    ...
    def get_payment_status(self, payment_id):
        ...
        status_map = {"captured": "PAYMENT_SUCCESS", "failed": "PAYMENT_DECLINED", "authorized": "PAYMENT_PENDING"}
        return {
