...
def detect_download_farming(profile, content_id):
    ...
    download_count = DownloadToken.objects.filter(user=profile, created_at__gte=last_hour).count()
    ...
    unique_ips = (
        DownloadToken.objects.filter(user=profile, content_id=content_id, created_at__gte=last_day)
...
def detect_payment_fraud(profile):
    ...
    failed_attempts = Purchase.objects.filter(
        user=profile, created_at__gte=last_10m, is_revoked=True  # Assuming revoked/failed tracking
    ).count()
