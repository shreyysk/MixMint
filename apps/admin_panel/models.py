...
class PromotionalOffer(models.Model):
    ...
    # Optional DJ owner: when set, this is a DJ-given offer shown on that DJ's
    # storefront + track pages (and cart when their items are inside).
    dj = models.ForeignKey("accounts.DJProfile", null=True, blank=True, on_delete=models.CASCADE, related_name="offers")
    # Placements
    ...
    @classmethod
    def active_for_dj(cls, dj):
        """Live DJ-given offers for storefront/track pages (date-window aware)."""
        from django.utils import timezone

        now = timezone.now()
        qs = cls.objects.filter(is_active=True, dj=dj).order_by("-created_at")
        live = []
        for offer in qs:
