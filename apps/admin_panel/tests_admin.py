...
class AdminViewTests(TestCase):
    def setUp(self):
        ...
        self.admin_user = User.objects.create_superuser(email="admin@mixmint.site", password="password123")
        self.admin_user.profile.role = "admin"
        self.admin_user.profile.save(update_fields=["role"])

        # Create a regular user (Profile auto-created via signal)
        self.regular_user = User.objects.create_user(email="user@mixmint.site", password="password123")
