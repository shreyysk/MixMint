    ...
    def __call__(self, request):
        ...
        if request.path.startswith("/download/"):
            ...
            if self._is_rapid_downloader(client_ip):
                return JsonResponse(
                    {"error": "Too many download requests. Please slow down.", "code": "RATE_LIMITED"}, status=429
                )
    ...
    def __call__(self, request):
        ...
        if request.user.is_authenticated and request.path.startswith("/api/"):
            if self._check_velocity(request.user):
                return JsonResponse(
                    {"error": "Unusual activity detected. Please try again later.", "code": "VELOCITY_CHECK"},
                    status=429,
                )
