...
def mobile_library(request):
    ...
    purchases = (
        Purchase.objects.filter(user=request.user.profile, status="paid")
        .select_related()
        .values("id", "content_type", "content_id", "created_at", "download_completed")
...
def mobile_dj_stats(request):
    ...
    data = {
        ...
        "sales_today": Purchase.objects.filter(seller=dj, status="paid", created_at__gte=today).count(),
...
def mobile_batch(request):
    ...
    for req in requests_data:
        ...
        if endpoint == "home":
            ...
            purchases = Purchase.objects.filter(user=request.user.profile, status="paid").values(
...
def mobile_track_detail(request, track_id):
    ...
    try:
        track = Track.objects.select_related("dj").get(id=track_id, is_active=True, is_deleted=False)
