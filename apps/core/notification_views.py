...
def register_push_subscription(request):
    ...
    return Response({"status": "subscribed" if created else "updated", "subscription_id": sub.id})
...
def unregister_push_subscription(request):
    ...
    if endpoint:
        PushSubscription.objects.filter(user=request.user.profile, endpoint=endpoint).delete()
    else:
        # Unregister all
    ...
    return Response(
        {
            ...
            "unread_count": InAppNotification.objects.filter(user=request.user.profile, is_read=False).count(),
        }
    )
...
def mark_notifications_read(request):
    ...
    if mark_all:
        InAppNotification.objects.filter(user=request.user.profile, is_read=False).update(
            is_read=True, read_at=timezone.now()
        )
    elif notification_ids:
        InAppNotification.objects.filter(user=request.user.profile, id__in=notification_ids).update(
            is_read=True, read_at=timezone.now()
        )
...
def track_experiment_event(request, experiment_name):
    ...
    success = ABTestingService.track_event(
        request.user.profile, experiment_name, event_type, value=value, metadata=metadata
    )
...
def admin_create_experiment(request):
    ...
    experiment = Experiment.objects.create(
        name=name, description=description, target_audience=target, traffic_percentage=traffic, status="draft"
    )
    ...
    return Response({"id": experiment.id, "name": experiment.name, "status": "created"})
