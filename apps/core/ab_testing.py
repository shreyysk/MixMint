...
class ABTestingService:
    ...
    def get_variant(cls, user_profile, experiment_name):
        ...
        # Check cache first
        cache_key = f"ab_{user_profile.pk}_{experiment_name}"
        ...
        assignment = (
            UserExperiment.objects.filter(user=user_profile, experiment=experiment).select_related("variant").first()
        )

        if assignment:
            result = {"variant": assignment.variant.name, "config": assignment.variant.config}
        ...
        result = {"variant": variant.name, "config": variant.config}
        cache.set(cache_key, result, 3600)
        return result
    ...
    def track_event(cls, user_profile, experiment_name, event_type, value=None, metadata=None):
        ...
        assignment = UserExperiment.objects.filter(user=user_profile, experiment__name=experiment_name).first()

        if not assignment:
            return False

        ExperimentEvent.objects.create(
            user_experiment=assignment, event_type=event_type, value=value, metadata=metadata or {}
        )
        return True
    ...
    def get_experiment_stats(cls, experiment_name):
        ...
        stats = {"name": experiment.name, "status": experiment.status, "variants": []}

        for variant in experiment.variants.all():
            assignments = UserExperiment.objects.filter(experiment=experiment, variant=variant)

            events = ExperimentEvent.objects.filter(
                user_experiment__variant=variant, user_experiment__experiment=experiment
            )
    ...
    def _assign_variant(cls, user_profile, experiment):
        ...
        # Create assignment
        UserExperiment.objects.create(user=user_profile, experiment=experiment, variant=selected)
