...
def dj_quick_stats(request):
    ...
    stats = {
        "earnings": {
            ...
            "today": str(
                Purchase.objects.filter(seller=dj, status="paid", created_at__gte=today).aggregate(
                    t=Sum("dj_earnings")
                )["t"]
                or 0
            ),
...
def smart_search(request):
    ...
    # Apply filters
    if q:
        tracks = tracks.filter(Q(title__icontains=q) | Q(dj__dj_name__icontains=q) | Q(description__icontains=q))
    ...
    total = tracks.count()
    start = (page - 1) * limit
    tracks = tracks[start : start + limit]  # noqa: E203 (black slice style)
...
def homepage_feed(request):
    ...
    trending = Track.objects.filter(is_active=True, is_deleted=False, created_at__gte=month_ago).order_by(
        "-download_count"
    )[:8]

    # New releases
    new_releases = Track.objects.filter(is_active=True, is_deleted=False, created_at__gte=week_ago).order_by(
        "-created_at"
    )[:8]

    # Featured DJs (verified with most sales)
    featured_djs = DJProfile.objects.filter(status="approved", is_verified=True).order_by("-popularity_score")[:6]
    ...
    feed = {
        ...
        "featured_djs": [
            {"id": str(d.id), "name": d.dj_name, "slug": d.slug, "verified": d.is_verified} for d in featured_djs
        ],
...
def admin_dashboard_stats(request):
    ...
    stats = {
        "revenue": {
            "today": str(
                Purchase.objects.filter(status="paid", created_at__gte=today).aggregate(t=Sum("price_paid"))["t"] or 0
            ),
            "this_week": str(
                Purchase.objects.filter(status="paid", created_at__gte=week_ago).aggregate(t=Sum("price_paid"))["t"]
                or 0
            ),
            "this_month": str(
                Purchase.objects.filter(status="paid", created_at__gte=month_ago).aggregate(t=Sum("price_paid"))["t"]
                or 0
            ),
            "commission_earned": str(
                Purchase.objects.filter(status="paid", created_at__gte=month_ago).aggregate(t=Sum("commission"))["t"]
                or 0
            ),
        },
        "users": {
...
def pending_dj_approvals(request):
    ...
    pending = (
        DJProfile.objects.filter(status__in=["pending", "pending_review"])
...
def quick_approve_dj(request, dj_id):
    ...
    return Response({"status": "approved", "dj_name": dj.dj_name, "message": f"{dj.dj_name} is now an approved DJ!"})
...
def get_tracks_optimized(filters=None, limit=20):
    ...
    qs = Track.objects.filter(is_active=True, is_deleted=False).select_related("dj", "dj__profile", "dj__profile__user")
