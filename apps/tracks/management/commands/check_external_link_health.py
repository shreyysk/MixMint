...
def validate_external_link(url):
    ...
    """
    ALLOWED_DOMAINS = ["drive.google.com", "docs.google.com", "mediafire.com", "www.mediafire.com"]
    ...
    if parsed.netloc not in ALLOWED_DOMAINS:
        return False, ("Only Google Drive and MediaFire links are accepted. " f"Got: {parsed.netloc}")
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            return False, ("Link is not publicly accessible. " "Make sure sharing is set to 'Anyone with the link'.")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # 1. Check Tracks
        external_tracks = Track.objects.filter(is_external_link=True, is_active=True, is_deleted=False)
        ...
        # 2. Check Albums
        external_albums = AlbumPack.objects.filter(is_external_link=True, is_active=True, is_deleted=False)
