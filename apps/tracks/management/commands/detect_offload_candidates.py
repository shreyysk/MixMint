...
class Command(BaseCommand):
    help = (
        "Phase 3 Feature 1: Detect underperforming tracks and free tracks on R2, notify DJs to offload to external link"
    )
    ...
    def handle(self, *args, **kwargs):
        ...
        paid_tracks = (
            Track.objects.filter(
                is_active=True, is_deleted=False, is_external_link=False, price__gt=0, created_at__lt=too_new_paid
            )
            .annotate(
            ...
            .filter(Q(sales_in_period=0) | Q(sales_in_period__lt=UNDERPERFORM_LOW_SALES_COUNT))
        )

        # Albums
        paid_albums = (
            AlbumPack.objects.filter(
                is_active=True, is_deleted=False, is_external_link=False, price__gt=0, created_at__lt=too_new_paid
            )
            .annotate(
                sales_in_period=Count(
        ...
        free_tracks = Track.objects.filter(
            is_active=True, is_deleted=False, is_external_link=False, price=0, created_at__lt=too_new_free
        )
        ...
        for content in all_candidates:
            ...
            already_notified = OffloadNotification.objects.filter(
                content_id=content.id, created_at__gte=timezone.now() - timedelta(days=30)
            ).exists()
