...
class IsDJ(BasePermission):
    ...
    def has_permission(self, request, view):
        return (
            ...
            and request.user.profile.role == "dj"
            and not request.user.profile.is_banned
            and not request.user.profile.is_frozen
            and hasattr(request.user.profile, "dj_profile")
            and request.user.profile.dj_profile.status == "approved"
        )
...
class IsAdmin(BasePermission):
    ...
    def has_permission(self, request, view):
        return request.user.is_authenticated and (
            request.user.is_staff or (hasattr(request.user, "profile") and request.user.profile.role == "admin")
        )
