...
def expire_inactive_accounts():
    ...
    inactive_profiles = Profile.objects.filter(user__last_login__lt=twelve_months_ago, is_frozen=False)
...
def update_dj_popularity_scores():
    ...
    for dj in djs:
        ...
        sales_count = Purchase.objects.filter(seller=dj, status="paid", created_at__gte=thirty_days_ago).count()

        # Get successful deliveries in last 30d
        delivery_count = Purchase.objects.filter(
            seller=dj, download_completed=True, created_at__gte=thirty_days_ago
        ).count()
