...
class TestTrackModel:
    ...
    def test_both_previews_accepted(self):
        from apps.tracks.models import Track
    ...
    def test_youtube_preview_without_url_rejected(self):
        ...
        dj = self._make_dj()
        t = Track(dj=dj, title="No URL", price=Decimal("50.00"), file_key="t.wav", preview_type="youtube")
        with pytest.raises(ValidationError):
            t.full_clean()
...
class TestCartDiscountTiers:
    ...
    def _create_cart_with_items(self, count, price_paise=10000):
        ...
        for i in range(count):
            CartItem.objects.create(cart=cart, content_type="track", content_id=i + 1, price=price_paise)
        return cart
...
class TestRevenueEngine:
    ...
    def _setup_dj(self, is_pro=False):
        ...
        dj = DJProfile.objects.create(
            profile=u.profile, dj_name="Rev DJ", slug=f'rev-dj-{"pro" if is_pro else "std"}', status="approved"
        )
        return dj
