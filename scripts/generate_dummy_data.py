...
def main():
    ...
    for data in djs_data:
        email = '{}@example.com'.format(data['name'].lower().replace(' ', ''))
        ...
        dj_profile, _ = DJProfile.objects.get_or_create(
            ...
            defaults={
                'dj_name': data['name'],
                'bio': 'This is the official profile of {}. Dropping heat since 2024.'.format(data['name']),
                'genres': data['genre'],
                'location': data['loc'],
                'status': 'approved',
    ...
    listeners = []
    for i in range(1, 6):
        email = 'listener{}@example.com'.format(i)
        user, created = User.objects.get_or_create(email=email)
        if created:
            user.set_password('StrongPass123!')
            user.save()
        ...
        profile, _ = Profile.objects.get_or_create(
            user=user,
            defaults={'role': 'user', 'full_name': 'Listener {}'.format(i)}
        )
        listeners.append(profile)

    # 3. Create Tracks
    genres = ['Techno', 'House', 'Trance', 'Dubstep', 'Lo-fi', 'Disco']
    track_names = ['Vibe', 'Flow', 'Beat', 'Pulse', 'Wave', 'Echo', 'Sync', 'Glitch']
    tracks = []
    for dj in djs:
        for i in range(1, 10):
            track_title = '{} - {} {}'.format(dj.dj_name, random.choice(track_names), i)
            ...
            track, created = Track.objects.get_or_create(
                ...
                defaults={
                    ...
                    'file_key': 'tracks/{}/{}.wav'.format(dj.slug, track_title.lower().replace(' ', '_')),
                    'is_active': True,
                    'is_deleted': False,
                }
            )
    ...
    # 3.5 Create Albums
    albums = []
    album_titles = ['Anthology', 'Chronicles', 'Vision', 'Rebirth']
    for dj in djs:
        for i in range(1, 3):
            album_title = '{} - {} Vol. {}'.format(dj.dj_name, random.choice(album_titles), i)
            album, created = AlbumPack.objects.get_or_create(
                dj=dj,
                title=album_title,
                defaults={
                    'price': Decimal('149.00') if i == 1 else Decimal('199.00'),
                    'description': 'A premium collection of secret tracks by {}.'.format(dj.dj_name),
                    'preview_type': 'youtube',
                    'youtube_url': 'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
                    'processing_status': 'completed',
                    'track_count': 5,
                    'file_key': 'albums/{}/{}.zip'.format(dj.slug, album_title.lower().replace(' ', '_')),
                    'is_active': True,
                    'is_deleted': False,
                }
            )
            ...
            for j in range(1, 6):
                AlbumTrack.objects.get_or_create(
                    album=album,
                    track_order=j,
                    defaults={'title': 'Album Track {}'.format(j), 'format': 'mp3'}
                )
    ...
    for dj in djs:
        DJAnnouncement.objects.create(
            ...
            content='Stay tuned for my upcoming release next week. It is going to be a banger!'
        )
    ...
    for dj in djs:
        ...
        if bundle_tracks.count() >= 2:
            bundle = Bundle.objects.create(
                dj=dj,
                title='{} Essential Pack'.format(dj.dj_name),
